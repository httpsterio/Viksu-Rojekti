...
export function useBoard() {
  ...
  const saveBoardConfig = async (newConfig: BoardConfig) => {
    try {
      ...
      if (
        activeFilters.value.epic &&
        !newConfig.epics.some((e) => e.name === activeFilters.value.epic)
      ) {
        activeFilters.value.epic = null
      }
      if (
        activeFilters.value.tag &&
        !newConfig.tags.some((t) => t.name === activeFilters.value.tag)
      ) {
        activeFilters.value.tag = null
      }
      if (
        activeFilters.value.priority &&
        activeFilters.value.priority > newConfig.priorities.length
      ) {
